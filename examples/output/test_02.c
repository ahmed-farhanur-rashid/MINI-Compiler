...
int main(void){
    ...
    __register_ptr(c);
    char* result;
    result = ({ char* __strtmp0 = NULL; size_t __need = strlen(a) + strlen(b) + strlen(c) + 1; __strtmp0 = (char*)malloc(__need); if(__strtmp0) __strtmp0[0]='\0'; if(a) strcat(__strtmp0, a);
if(b) strcat(__strtmp0, b);
if(c) strcat(__strtmp0, c);
__register_ptr(__strtmp0); __strtmp0; });
    __register_ptr(result);
    { fputs(result ? result : "", stdout); }
    __cleanup_all();
    return 0;
}
